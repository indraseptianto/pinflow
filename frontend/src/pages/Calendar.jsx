...
const formatScheduleTime = (value) => value ? dayjs(value).format('MMM D, YYYY h:mm A') : 'Not scheduled'
const displayBoard = (pin) => pin.board_name || pin.board || pin.board_id || 'No board'
const displayScheduleStatus = (pin) => pin.schedule_status || pin.status || 'draft'
...
function Calendar() {
  ...
  const visiblePins = useMemo(() => {
    const filteredPins = statusFilter === 'all'
      ? pins
      : pins.filter((pin) => (pin.status || 'draft') === statusFilter)

    return [...filteredPins].sort((first, second) => {
      const firstTitle = (first.title || '').toLowerCase()
      const secondTitle = (second.title || '').toLowerCase()
      const comparison = firstTitle.localeCompare(secondTitle)
      return titleSort === 'az' ? comparison : -comparison
    })
  }, [pins, statusFilter, titleSort])
  ...
  return (
    <section className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
      ...
      <div className="mb-4 flex flex-col justify-between gap-3 rounded-[1.5rem] border border-slate-200 bg-white p-3 sm:flex-row sm:items-center">
        <div className="flex flex-wrap gap-2">
          {statusFilters.map((filter) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-bold capitalize transition ${statusFilter === filter ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
            >
              {filter}
            </button>
          ))}
        ...
        <button
          type="button"
          onClick={() => setTitleSort((current) => current === 'az' ? 'za' : 'az')}
        ...
        >
          {titleSort === 'az' ? <ArrowDownAZ size={16} /> : <ArrowUpAZ size={16} />}
          {titleSort === 'az' ? 'A-Z' : 'Z-A'}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-3xl border border-slate-200">
          {pins.length === 0 ? <p className="p-8 text-center text-slate-500">No pins yet.</p> : visiblePins.length === 0 ? <p className="p-8 text-center text-slate-500">No pins match this filter.</p> : visiblePins.map((pin) => {
            const id = pin.id || pin.pin_id
            const status = pin.status || 'draft'
            return (
              <div key={id} className="grid gap-4 border-b border-slate-100 p-4 last:border-b-0 md:grid-cols-[32px_64px_minmax(220px,1fr)_110px_190px_160px_130px_170px] md:items-center">
                ...
                <div>
                  ...
                  {pin.postfast_post_id && <p className="mt-1 text-xs text-slate-400">PostFast: {pin.postfast_post_id}</p>}
                </div>
                <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${badgeColors[status] || badgeColors.draft}`}>{status}</span>
                <div className="text-sm text-slate-600">
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Scheduled for</p>
                  <p>{formatScheduleTime(pin.scheduled_at)}</p>
                </div>
                <div className="text-sm text-slate-600">
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Board</p>
                  <p className="truncate" title={displayBoard(pin)}>{displayBoard(pin)}</p>
                </div>
                <div>
                  <p className="mb-1 text-xs font-bold uppercase tracking-wide text-slate-400">Schedule status</p>
                  <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${scheduleBadgeColors[displayScheduleStatus(pin)] || scheduleBadgeColors.draft}`}>{displayScheduleStatus(pin)}</span>
                  {pin.schedule_error && <p className="mt-1 text-xs text-red-600">{pin.schedule_error}</p>}
                </div>
